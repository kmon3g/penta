...
router.post('/', function(req,res,next){ //all --> post
  ...
  var query=connection.query('select project_name from project where project_name=?', data.name, function(err, results){
   ...
   if(!results[0]){
    var query=connection.query('insert into project values((select max(project_id)+1 from project pj),?)',data.name, function (err, results) {
      //insert 서브쿼리 테이블에 별명 없을 경우 에러
      if (err) {
       console.log(err);
     }
